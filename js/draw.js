...
function showHighlight() {
	if ((glbData.debug & Debug.fastMode) == 0) {
		//绘制高亮记号（仅用于调试）
		var posX, posY;
		var cellSide = Math.round(windowSizeMap / glbData.mapCellSize); //边长
		...
		else if (cityList.length > 0 && glbData.hltCityId != CityId.none && cityList[glbData.hltCityId] != null && mapMain.data.cells[glbData.hltPosX][glbData.hltPosY].cBase == CityBase.center){
			posX = glbData.hltPosX;
			posY = glbData.hltPosY;
